...
def aplicar_js_graficos_mobile() -> None:
    """Ajusta legendas (mobile) e eixo X esparsado no celular; desktop intacto."""
...
(function () {
  const BP = 768;
  const TICKS_MOB = [1, 9.5, 19, 28.5, 38];
  ...
  function legendCount(gd) {
    return gd.data.filter(function (t) {
      return t.showlegend !== false && t.name;
    }).length;
  }

  function isRodadaChart(gd) {
    var ax = gd.layout && gd.layout.xaxis;
    if (!ax || !ax.title) return false;
    var t = ax.title.text || ax.title;
    return t === "Rodada";
  }

  function rMaxFromLayout(gd) {
    var r = gd.layout && gd.layout.xaxis && gd.layout.xaxis.range;
    if (r && r.length === 2) return r[1] - 0.5;
    return 38;
  }

  function saveOrig(gd) {
    if (store.has(gd)) return;
    store.set(gd, {
      marginT: (gd.layout.margin && gd.layout.margin.t) || 80,
      legend: Object.assign({}, gd.layout.legend || {}),
      title: Object.assign({}, gd.layout.title || {}),
      xaxis: Object.assign({}, gd.layout.xaxis || {}),
    });
  }

  function applyMobile(gd) {
    saveOrig(gd);
    var n = Math.max(legendCount(gd), 1);
    var cols = Math.max(1, Math.floor(window.innerWidth / 120));
    var rows = Math.ceil(n / cols);
    var figH = (gd.layout && gd.layout.height) || 520;
    var titlePx = 34;
    var legendPx = rows * 20 + 4;
    var gap = 8;
    var marginTop = titlePx + legendPx + gap + 16;
    var patch = {
      "margin.t": marginTop,
      "title.yref": "paper",
      "title.y": 1,
      ...
      "legend.yref": "paper",
      "legend.yanchor": "top",
      "legend.y": 1 - (titlePx + gap) / figH,
      "legend.x": 0,
      "legend.xanchor": "left",
      "legend.tracegroupgap": 2,
    };
    if (isRodadaChart(gd)) {
      var rmax = rMaxFromLayout(gd);
      var ticks = TICKS_MOB.filter(function (t) { return t <= rmax + 0.01; });
      patch["xaxis.tickmode"] = "array";
      patch["xaxis.tickvals"] = ticks;
      patch["xaxis.ticktext"] = ticks.map(function (t) {
  ...
  function applyDesktop(gd) {
    var orig = store.get(gd);
    if (!orig) return;
    var patch = {
      "margin.t": orig.marginT,
      "legend.orientation": orig.legend.orientation || "h",
      "legend.yanchor": orig.legend.yanchor || "bottom",
      "legend.y": orig.legend.y != null ? orig.legend.y : 1.02,
      "legend.x": orig.legend.x != null ? orig.legend.x : 0,
      "legend.xanchor": orig.legend.xanchor || "left",
      "legend.yref": orig.legend.yref || "paper",
      "legend.tracegroupgap": orig.legend.tracegroupgap || 0,
      "title.yref": orig.title.yref || undefined,
      "title.y": orig.title.y != null ? orig.title.y : undefined,
      "title.yanchor": orig.title.yanchor || undefined,
      "title.pad.t": orig.title.pad && orig.title.pad.t != null ? orig.title.pad.t : undefined,
      "title.pad.b": orig.title.pad && orig.title.pad.b != null ? orig.title.pad.b : undefined,
    };
    if (isRodadaChart(gd)) {
      var rmax = rMaxFromLayout(gd);
      patch["xaxis.tickmode"] = "linear";
      patch["xaxis.dtick"] = 1;
      patch["xaxis.tick0"] = 1;
      patch["xaxis.tickvals"] = null;
      patch["xaxis.ticktext"] = null;
      patch["xaxis.range"] = [0.5, Math.min(38, rmax) + 0.5];
    }
    window.Plotly.relayout(gd, patch);
  }
  ...
  function adjustAll() {
    ...
    document.querySelectorAll('[data-testid="stPlotlyChart"] .js-plotly-plot').forEach(function (gd) {
      if (!gd.data) return;
      if (isMobile()) applyMobile(gd);
      else applyDesktop(gd);
    });
  }

  function boot() {
    if (window.Plotly) adjustAll();
    else setTimeout(boot, 150);
  }

  window.addEventListener("resize", function () { setTimeout(adjustAll, 120); });
  new MutationObserver(function () { setTimeout(adjustAll, 350); })
